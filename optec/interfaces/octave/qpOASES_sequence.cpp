 ...
 *	\author Hans Joachim Ferreau, Christian Kirches, Andreas Potschka
 *	\version 3.0beta
 *	\date 2007-2014
 *
	...
	if ( ( strcmp( typeString,"i" ) == 0 ) || ( strcmp( typeString,"I" ) == 0 ) )
	{
		...
		if ( mxIsSparse( prhs[1] ) != 0 )
		{
			...
			for (j = 0; j < nV; j++) 
			{
				...
				for (; i < oct_jc[j+1]; i++) 
				{
					globalQP->Hir[i + nfill] = (sparse_int_t)(oct_ir[i]);
					globalQP->Hv[i + nfill] = (real_t)(v[i]);
				}
			}
			globalQP->Hjc[nV] = (sparse_int_t)(oct_jc[nV]) + nfill;
		...
		if ( nC > 0 )
		{
			...
			if ( mxIsSparse( prhs[3] ) != 0 )
			{
				...
				for (i = 0; i < oct_jc[nV]; i++) globalQP->Air[i] = (sparse_int_t)(oct_ir[i]);
				for (i = 0; i < nV + 1; i++) globalQP->Ajc[i] = (sparse_int_t)(oct_jc[i]);

				/* copy values, too */
				globalQP->Av = new real_t[globalQP->Ajc[nV]];
				for (long i = 0; i < globalQP->Ajc[nV]; i++) globalQP->Av[i] = (real_t)(v[i]);
	...
	if ( ( strcmp( typeString,"m" ) == 0 ) || ( strcmp( typeString,"M" ) == 0 ) )
	{
		...
		if ( nC > 0 )
		{
			...
			if ( mxIsSparse( prhs[4] ) != 0 )
			{
				...
				/* copy values, too */
				globalQP->Av = new real_t[globalQP->Ajc[nV]];
				for (i = 0; i < globalQP->Ajc[nV]; i++) globalQP->Av[i] = (real_t)(v[i]);
