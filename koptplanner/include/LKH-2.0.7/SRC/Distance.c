...
int Distance_ATSP(Node * Na, Node * Nb)
{
    ...
    if (abs(Na->Id - Nb->Id) == n)
        return 0;
    return Na->Id < Nb->Id ? c_callback_function(Na->Id - 1, Nb->Id - 1 - n) : c_callback_function(Nb->Id - 1, Na->Id - 1 - n);// Na->C[Nb->Id - n] : Nb->C[Na->Id - n];
}
...
int Distance_RRT_2D(Node * Na, Node * Nb)
{
    int ret = c_callback_function(Na->Id - 1, Nb->Id - 1);
    assert(ret>=0);
    return ret;
}

int Distance_RRT_3D(Node * Na, Node * Nb)
{
    int ret = c_callback_function(Na->Id - 1, Nb->Id - 1);
    assert(ret>=0);
    return ret;
