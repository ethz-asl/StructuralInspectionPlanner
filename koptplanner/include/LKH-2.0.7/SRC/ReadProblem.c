...
static void CheckSpecificationPart()
{
    ...
    if (WeightType == EXPLICIT && WeightFormat == FUNCTION)
        eprintf("Conflicting EDGE_WEIGHT1_TYPE and EDGE_WEIGHT_FORMAT");
    if (WeightType != EXPLICIT
        && (WeightType != SPECIAL || CoordType != NO_COORDS)
        && WeightType != -1 && WeightFormat != -1
        && WeightFormat != FUNCTION)
        eprintf("Conflicting EDGE_WEIGHT2_TYPE and EDGE_WEIGHT_FORMAT");
...
static void Read_EDGE_WEIGHT_TYPE()
{
    ...
    if (!strcmp(EdgeWeightType, "ATT")) {
        ...
        c = c_EUC_3D;
        CoordType = THREED_COORDS;
    } else if (!strcmp(EdgeWeightType, "RRT_2D")) {
        WeightType = EUC_2D;
        Distance = Distance_RRT_2D;
        c = c_EUC_2D;
        CoordType = TWOD_COORDS;
    } else if (!strcmp(EdgeWeightType, "RRT_3D")) {
        WeightType = EXPLICIT;
        Distance = Distance_RRT_3D;
        c = c_EUC_3D;
        CoordType = THREED_COORDS;
